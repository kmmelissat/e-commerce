...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  return (
    <Link href={`/product/${product.slug}`} className='block'>
      <Card className='group relative overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 bg-white dark:bg-gray-900 cursor-pointer'>
        {/* Image Container */}
        <div className='relative overflow-hidden'>
          ...
          <div className='absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300'>
            <button
              className='w-8 h-8 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center shadow-lg hover:bg-[#F8E559] hover:text-black transition-all duration-200'
              onClick={(e) => e.preventDefault()}
            >
              <Heart className='h-4 w-4' />
            </button>
            <button
              className='w-8 h-8 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center shadow-lg hover:bg-[#864AF9] hover:text-white transition-all duration-200'
              onClick={(e) => e.preventDefault()}
            >
        ...
        <CardContent className='p-4 space-y-3'>
          ...
          {product.stock > 0 && (
            <button
              className='w-full bg-gradient-to-r from-[#864AF9] to-[#4C1D95] hover:from-[#7C3AED] hover:to-[#3B3486] text-white py-2 px-4 rounded-lg font-medium text-sm flex items-center justify-center gap-2 transition-all duration-200 hover:shadow-lg'
              onClick={(e) => e.preventDefault()}
            >
